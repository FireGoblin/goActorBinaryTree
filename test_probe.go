...
func maketestProbe() *testProbe {
	x := testProbe{make(chan operation), make(chan operationReply, 1024), MakeBinaryTreeSet(), make(map[int]bool), make(map[int]bool), replyTracker{make(map[int]bool), &sync.Mutex{}}, 1, rand.New(rand.NewSource(777)), 0}
	return &x
}
